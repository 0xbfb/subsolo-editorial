...
const tuple2 = (value, path) => {
    ...
    if (items.length !== 2)
        fail(`${path} deve ter duas linhas`);
    return Object.freeze([
        nonEmptyString(items[0], `${path}[0]`),
        nonEmptyString(items[1], `${path}[1]`),
    ]);
};
const stringList = (value, path) => Object.freeze(asArray(value, path).map((item, index) => nonEmptyString(item, `${path}[${index}]`)));
const storyList = (value, path) => Object.freeze(asArray(value, path).map((item, index) => {
    ...
    return Object.freeze({
        ...
        href: typeof record.href === 'string' && record.href.trim() !== '' ? record.href : undefined,
    });
}));
export const parseHomePageData = (value) => {
    ...
    if (!allowedStates.includes(stateKind))
        fail('hero.stateKind não é reconhecido');
    const navigation = Object.freeze(asArray(root.navigation, 'navigation').map((item, index) => {
