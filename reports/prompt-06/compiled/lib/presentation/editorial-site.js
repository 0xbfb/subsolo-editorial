...
const stateKind = (value, path) => {
  ...
  if (!allowed.includes(parsed)) fail(path, 'estado visual desconhecido');
  return parsed;
};
const block = (value, path) => {
  ...
  if (!allowed.includes(kind)) fail(`${path}.kind`, 'bloco desconhecido');
...
export const parseEditorialSiteData = (value) => {
  ...
  const sources = Object.freeze(
    list(root.sources, 'sources').map((entry, index) => {
  ...
  const stories = Object.freeze(
    list(root.stories, 'stories').map((entry, index) => {
  ...
  const documents = Object.freeze(
    list(root.documents, 'documents').map((entry, index) => {
...
export const navigationFor = (currentHref) =>
  Object.freeze([
...
export const sourcesFor = (data, publication) =>
  publication.sourceIds.map((id) => {
    const source = data.sources.find((item) => item.id === id);
    if (!source) throw new Error(`Fonte inexistente: ${id}`);
    return source;
  });
export const connectionsFor = (data, publication) =>
