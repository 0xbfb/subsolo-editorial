...
test('ciclo integrado cria caminho, envia, confirma, baixa e registra sem URL privada', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (
      ...
      target.searchParams.get('uploadType') === 'resumable'
    ) {
      return new Response('', {
        status: 200,
        headers: { location: 'https://upload.example/integrated' },
      });
    }
    if (target.toString() === 'https://upload.example/integrated' && method === 'PUT') {
      uploadCount += 1;
      archived = true;
      return json({ id: fileId }, 200);
    }
    if (
      target.pathname === `/drive/v3/files/${fileId}` &&
  ...
  const second = await applyPackageArchive({
    packagePath,
    rootFolderId: 'root',
    receiptPath: secondReceiptPath,
    adapter,
    now: () => new Date('2026-07-20T18:35:00.000Z'),
  });
