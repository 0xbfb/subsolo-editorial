...
const capture = async (fn) => {
  try {
    ...
    assert.ok(error instanceof GoogleDriveArchiveFailure, String(error));
    return error;
  }
};
...
test('valida raiz e cria somente a hierarquia técnica ausente', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (target.pathname === '/drive/v3/files/root')
      return json({
        id: 'root',
        name: 'SUBSOLO',
        mimeType: folderMime,
    ...
    if (target.pathname === '/drive/v3/files' && (options.method ?? 'GET') === 'GET')
      return json({ files: [] });
  ...
  assert.deepEqual(
    result.segments.map((item) => item.status),
...
test('dry-run marca pastas faltantes sem criar recursos', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    const target = new URL(String(url));
    if (target.pathname.endsWith('/root'))
  ...
  assert.deepEqual(
    plan.segments.map((item) => item.status),
    ['create', 'create', 'create', 'create'],
  );
});
      ...
      files: [
        {
          ...
          parents: ['month'],
          appProperties: { subsolo_package_sha256: 'a'.repeat(64) },
          size: '10',
          trashed: false,
        },
      ],
...
test('upload resumível envia chunks e conclui com metadata', async () => {
  ...
  const result = await adapter.uploadResumable({
    folderId: 'month',
    name: 'edition.zip',
    bytes: Buffer.alloc(total),
    metadata: { subsolo_package_sha256: 'b'.repeat(64) },
  });
  assert.equal(result.id, 'uploaded-file');
  assert.deepEqual(ranges, [
    `bytes 0-262143/${total}`,
    `bytes 262144-524287/${total}`,
    `bytes 524288-599999/${total}`,
  ]);
});
...
test('interrupção consulta a sessão e retoma do último byte confirmado', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    ranges.push(range);
    if (range?.startsWith('bytes 0-') && !failed) {
      failed = true;
  ...
  const result = await adapter.uploadResumable({
    ...
    metadata: { subsolo_package_sha256: 'c'.repeat(64) },
  });
...
test('raiz sem autorização gera erro acionável e não é tratada como pasta ausente', async () => {
  const adapter = createGoogleDriveArchiveAdapter({
    tokenProvider,
    fetchImpl: async () => json({ error: { status: 'PERMISSION_DENIED' } }, 403),
  });
...
test('conta de serviço solicita drive.file por padrão', async () => {
  ...
  await writeFile(
    credentialPath,
    JSON.stringify({
      client_email: 'drive-test@example.iam.gserviceaccount.com',
...
test('quota 429 é repetida e termina quando o Drive responde', async () => {
  ...
  const adapter = createGoogleDriveArchiveAdapter({
    ...
    fetchImpl: async () => {
      attempts += 1;
      if (attempts === 1)
        return json({ error: { errors: [{ reason: 'rateLimitExceeded' }] } }, 429);
      return json({ id: 'root', name: 'SUBSOLO', mimeType: folderMime, trashed: false });
    },
  });
...
test('sessão que não confirma a conclusão é rejeitada como upload parcial', async () => {
  ...
  const fetchImpl = async (url, options = {}) => {
    if (String(url).includes('uploadType=resumable'))
      return new Response('', {
        status: 200,
        headers: { location: 'https://upload.example/session/partial' },
      });
    const range = options.headers['content-range'];
    if (range === `bytes 0-${total - 1}/${total}`)
      return new Response('', { status: 308, headers: { range: `bytes=0-${total - 1}` } });
  ...
  const error = await capture(() =>
    adapter.uploadResumable({
      folderId: 'month',
      name: 'edition.zip',
      bytes: Buffer.alloc(total),
      metadata: { subsolo_package_sha256: 'd'.repeat(64) },
    }),
  );
