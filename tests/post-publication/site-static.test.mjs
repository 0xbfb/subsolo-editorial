...
const routeSource = await readFile(
  new URL('../../src/pages/[year]/[month]/[day]/[slug].astro', import.meta.url),
  'utf8',
);
const correctionComponent = await readFile(
  new URL('../../src/components/article/CorrectionNotice.astro', import.meta.url),
  'utf8',
);
const tombstoneComponent = await readFile(
  new URL('../../src/components/article/WithdrawalTombstone.astro', import.meta.url),
  'utf8',
);
...
test('nota pública mostra tipo, impacto e cadeia de revisão', () => {
  ...
  assert.match(
    correctionComponent,
    /r\{correction\.previousRevision\} → r\{correction\.newRevision\}/,
  );
...
test('retirada permanece no arquivo e na busca sem indexar o corpo ocultado', () => {
  const entries = createDiscoveryEntries({
    ...
    archiveRecords: parseArchiveRecords(archive),
  });
