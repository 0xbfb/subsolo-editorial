...
const load = async (name) =>
  JSON.parse(
    await readFile(
      new URL(`../../fixtures/post-publication/${name}.json`, import.meta.url),
      'utf8',
    ),
  );
...
test('dry-run descreve revisão, pacote e PR sem efeitos', async () => {
  ...
  assert.deepEqual(plan.ordered_steps.slice(0, 4), [
...
test('atualização factual disfarçada de update é rejeitada', async () => {
  ...
  assert.throws(
    () => applyPostPublicationChange({ state, payload }),
...
test('orquestração arquiva pacote antes de qualquer efeito Git e atualiza Sheets', async () => {
  ...
  const ports = {
    ...
    git: {
      ...
      createPullRequest: async () => {
        events.push(['pr']);
        return { url: 'https://github.example/subsolo/pull/2' };
      },
    },
  };
  ...
  assert.equal(
    events.findIndex(([name]) => name === 'drive') <
      events.findIndex(([name]) => name === 'branch'),
    true,
  );
...
test('falha de arquivo técnico impede branch e PR', async () => {
  ...
  const ports = {
    sheets: {
      readPostPublicationState: async () => ({ revision: 1, canonical_path: state.canonical_path }),
      updatePostPublicationState: async () => events.push(['sheet']),
    },
    packages: {
    ...
    git: {
      createBranch: async () => events.push(['branch']),
      commitPostPublication: async () => events.push(['commit']),
      findOpenPullRequest: async () => null,
      createPullRequest: async () => events.push(['pr']),
    },
  };
  await assert.rejects(
    () => executePostPublicationRun({ state, payload, ports, mode: 'apply' }),
    /não foi confirmado/,
  );
...
test('pacotes r2 e r3 preservam cadeia, revisão anterior e tombstone', async () => {
  ...
  const r2Corrections = JSON.parse(
    r2.entries.get('publications/pub_MB76FYPQZ5RTPENBAR5611AJJ0/corrections.json').toString('utf8'),
  );
  const r3Corrections = JSON.parse(
    r3.entries.get('publications/pub_MB76FYPQZ5RTPENBAR5611AJJ0/corrections.json').toString('utf8'),
  );
  assert.equal(r2Corrections.at(-1).type, 'correcao-factual');
  assert.equal(r3Corrections.at(-1).type, 'retirada');
  const withdrawnBody = r3.entries
    .get('publications/pub_MB76FYPQZ5RTPENBAR5611AJJ0/publication.md')
    .toString('utf8');
