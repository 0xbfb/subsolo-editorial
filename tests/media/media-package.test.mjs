...
test('pacote inclui somente derivados em media/derived', async () => {
  ...
  await copyFile(
    join('fixtures/media/golden/portraits/cora-fixture', filename),
  ...
  assert.ok(
    [...inspected.entries.keys()].every(
      (name) => !/(^|\/)(?:originals?|masters?)(\/|$)/i.test(name),
    ),
  );
...
test('workspace com original é rejeitado antes do pacote', async () => {
  ...
  const workspace = join(temp, 'workspace');
  await cp('fixtures/packager/edition-r1', workspace, { recursive: true });
  const target = join(workspace, 'media/originals');
  await mkdir(target, { recursive: true });
