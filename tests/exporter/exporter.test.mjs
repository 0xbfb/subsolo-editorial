...
const capture = async (fn) => {
  try {
    ...
    assert.ok(error instanceof ExportFailure);
    return error;
  }
};
...
test('dry-run é determinístico e não grava destino', async () => {
  ...
  assert.equal(a.files.length, 5);
  assert.match(a.publication_id, /^pub_[0-9A-HJKMNP-TV-Z]{26}$/);
});
...
test('apply gera os cinco arquivos públicos esperados', async () => {
  ...
  const result = await applyExport({
    sheetPath: validSheet,
    documentPath: validDocument,
    destination: target,
  });
  ...
  assert.match(markdown, /\| Horário \| Evento \|/);
...
test('mesma entrada gera bytes iguais ao golden', async () => {
  ...
  for (const [name, content] of output.files)
    assert.equal(content, await readFile(`fixtures/exporter/golden/${name}`, 'utf8'), name);
});
...
test('proveniência pública usa hashes e não expõe IDs do Google', async () => {
  ...
  assert.doesNotMatch(
    text,
    /document_id|document_revision|fixture-transporte|rev-17|docs\.google|drive\.google/i,
  );
});
...
test('allowlist remove campos privados de fontes, correções e mídia', async () => {
  ...
  row.correcoes = [
    {
      type: 'esclarecimento',
      published_at: '2026-07-20T16:00:00-03:00',
      summary: 'Descrição pública.',
      impact: 'Sem mudança de conclusão.',
      previous_revision: 1,
      new_revision: 2,
      notes_private: 'não exportar',
    },
  ];
  row.midia = [
    {
      ...
      derivatives: [
        {
          path: '/media/teste.webp',
          width: 1200,
          height: 675,
          format: 'webp',
          notes_private: 'ignorar',
        },
      ],
      notes_private: 'não exportar',
    },
...
test('rejeita sugestão pendente', async () => {
  const sheet = JSON.parse(await readFile(validSheet));
  sheet.document_id = 'fixture-suggestion';
  sheet.titulo = 'Documento pendente';
  const doc = JSON.parse(await readFile(fixture('document-suggestion.json')));
  const error = await capture(async () => buildExport({ row: sheet, document: doc }));
  assert.equal(error.code, 'SUBSOLO_EXPORT_SUGGESTIONS_PENDING');
});
test('rejeita comentários internos', async () => {
  const sheet = JSON.parse(await readFile(validSheet));
  sheet.document_id = 'fixture-comment';
  sheet.titulo = 'Documento comentado';
  const doc = JSON.parse(await readFile(fixture('document-comment.json')));
  const error = await capture(async () => buildExport({ row: sheet, document: doc }));
  assert.equal(error.code, 'SUBSOLO_EXPORT_COMMENTS_PENDING');
});
test('rejeita link privado', async () => {
  const sheet = JSON.parse(await readFile(validSheet));
  sheet.document_id = 'fixture-private';
  sheet.titulo = 'Documento privado';
  const doc = JSON.parse(await readFile(fixture('document-private-link.json')));
  const error = await capture(async () => buildExport({ row: sheet, document: doc }));
  assert.equal(error.code, 'SUBSOLO_EXPORT_PRIVATE_URL');
});
test('rejeita metadado obrigatório ausente', async () => {
  const error = await capture(() =>
    planExport({
      sheetPath: fixture('sheet-missing-title.json'),
      documentPath: validDocument,
      destination: '/tmp/x',
    }),
  );
  assert.equal(error.code, 'SUBSOLO_EXPORT_METADATA_MISSING');
});
test('rejeita revisão factual pendente', async () => {
  const error = await capture(() =>
    planExport({
      sheetPath: fixture('sheet-review-pending.json'),
      documentPath: validDocument,
      destination: '/tmp/x',
    }),
  );
  assert.equal(error.code, 'SUBSOLO_EXPORT_REVIEW_PENDING');
});
test('rejeita conflito de título', async () => {
  const error = await capture(() =>
    planExport({
      sheetPath: fixture('sheet-title-conflict.json'),
      documentPath: validDocument,
      destination: '/tmp/x',
    }),
  );
  assert.equal(error.code, 'SUBSOLO_EXPORT_TITLE_CONFLICT');
});
...
test('imagem de capa é reconstruída por allowlist', async () => {
  ...
  row.imagem_publica = {
    src: '/media/capa.webp',
    alt: 'Descrição pública.',
    notes_private: 'segredo',
  };
  const output = buildExport({ row, document });
...
test('rejeita destino existente sem overwrite', async () => {
  const destination = await mkdtemp(path.join(tmpdir(), 'subsolo-existing-'));
  const error = await capture(() =>
    applyExport({ sheetPath: validSheet, documentPath: validDocument, destination }),
  );
  assert.equal(error.code, 'SUBSOLO_EXPORT_DESTINATION_EXISTS');
});
