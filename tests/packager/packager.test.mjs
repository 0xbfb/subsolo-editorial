...
const capture = async (fn) => {
  try {
    ...
    assert.ok(error instanceof PackageFailure, String(error));
    return error;
  }
};
...
test('pacote contém estrutura pública e checksums válidos', async () => {
  ...
  for (const required of [
    ...
    'publications/pub_MB76FYPQZ5RTPENBAR5611AJJ0/publication.md',
  ])
...
test('nome padrão inclui timestamp, fuso e revisão', async () => {
  const plan = await planPackage({
    workspace: r1,
    destination: await mkdtemp(path.join(tmpdir(), 'subsolo-name-')),
  });
...
test('restore dry-run não grava e apply reconstrói os arquivos', async () => {
  ...
  const zip = path.join(dir, 'edition.zip');
  await applyPackage({ workspace: r1, destination: zip });
  const target = path.join(dir, 'restored');
  const dry = await planRestore({ packagePath: zip, destination: target });
  assert.equal(dry.checksums_valid, true);
...
test('restore rejeita pacote corrompido', async () => {
  const built = await buildPackage({ workspace: r1 });
  const corrupt = Buffer.from(built.zip);
  corrupt[80] ^= 0xff;
  const dir = await mkdtemp(path.join(tmpdir(), 'subsolo-corrupt-'));
  const zip = path.join(dir, 'bad.zip');
  await writeFile(zip, corrupt);
  const error = await capture(() =>
    planRestore({ packagePath: zip, destination: path.join(dir, 'out') }),
  );
...
test('r2 exige pacote anterior e valida supersedes', async () => {
  ...
  let error = await capture(() =>
    planPackage({ workspace: r2, destination: path.join(dir, 'r2.zip') }),
  );
  assert.equal(error.code, 'SUBSOLO_PACKAGE_PREVIOUS_REQUIRED');
  const previous = path.join(dir, 'r1.zip');
  await applyPackage({ workspace: r1, destination: previous });
  const plan = await planPackage({
    workspace: r2,
    destination: path.join(dir, 'r2.zip'),
...
test('r2 rejeita supersedes divergente', async () => {
  ...
  const previous = path.join(dir, 'r1.zip');
  await applyPackage({ workspace: r1, destination: previous });
  const copy = path.join(dir, 'r2');
  await mkdir(copy); // fixture será clonada no helper da implementação se necessário
  const edition = JSON.parse(await readFile(path.join(r2, 'edition.json'), 'utf8'));
  edition.supersedes = 'run_inexistente';
...
test('destino existente idêntico é detectado como duplicado', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'subsolo-duplicate-'));
  const zip = path.join(dir, 'edition.zip');
  await applyPackage({ workspace: r1, destination: zip });
...
test('rejeita revisão inválida', async () => {
  const edition = JSON.parse(await readFile(path.join(r1, 'edition.json'), 'utf8'));
  edition.revision = 0;
  const error = await capture(() =>
    planPackage({ workspace: r1, destination: '/tmp/x.zip', editionOverride: edition }),
  );
...
test('rejeita caminho de publicação fora da raiz permitida', async () => {
  const edition = JSON.parse(await readFile(path.join(r1, 'edition.json'), 'utf8'));
  edition.publications[0].path = '../private';
  const error = await capture(() =>
    planPackage({ workspace: r1, destination: '/tmp/x.zip', editionOverride: edition }),
  );
...
test('restore rejeita path traversal forjado no ZIP', async () => {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), 'subsolo-traversal-'));
  const zip = path.join(dir, 'bad.zip');
  await writeFile(zip, malicious);
  const error = await capture(() =>
    planRestore({ packagePath: zip, destination: path.join(dir, 'out') }),
  );
  assert.equal(error.code, 'SUBSOLO_PACKAGE_PATH_INVALID');
