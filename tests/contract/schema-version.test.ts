...
describe('versionamento de schema', () => {
  ...
  it('rejeita major desconhecida', () => expect(migrateSchemaDocument({ schema_version: '2.0.0' }).ok).toBe(false));
  it('executa migração explicitamente registrada', () => {
    const result = migrateSchemaDocument({ schema_version: '0.9.0', summary: 'x' }, '1.0.0', [{ from: '0.9.0', to: '1.0.0', migrate: (doc) => ({ ...doc, description: doc.summary }) }]);
