...
const fixture = async (name) => JSON.parse(await readFile(new URL(`../../fixtures/ingestion/${name}`, import.meta.url), 'utf8'));
const memoryRepository = (initial=[]) => { const rows=[...initial]; return { rows, async list(){return [...rows];}, async insert(p){rows.push(p);return {inserted:true};} }; };
const memoryQuarantine = () => { const rows=[]; return { rows, async record(v){rows.push(v);} }; };
const fixedNow = () => new Date('2026-07-20T18:00:00-03:00');

test('canonical URL removes tracking and sorts query params', () => {
  assert.equal(canonicalizeUrl('https://WWW.Example.com/a/?utm_source=rss&b=2&a=1#top'),'https://example.com/a?a=1&b=2');
});

test('FreshRSS preserves publication date separately from fact date', () => {
  const entry=normalizeFreshRssItem({title:'Fato',url:'https://example.com/fato',published_at:'2026-07-20T12:00:00-03:00',fact_occurred_at:'2026-07-19T22:00:00-03:00'});
...
test('release remains an unverified communication', () => {
  const entry=normalizeFreshRssItem({title:'Press release: produto',url:'https://corp.example/release'});
...
test('same fingerprint is an exact duplicate', () => {
  const entry=normalizeFreshRssItem({title:'Mesmo título',url:'https://example.com/x',origin:{title:'Origem'}});
  const pitch=createPreliminaryPitch(entry);
...
test('trusted Gmail attachment is quarantined but not downloaded or opened', async () => {
  ...
  const repository=memoryRepository(); const quarantine=memoryQuarantine();
  const report=await ingestEditorialEntries({source:'gmail',entries:input.messages,repository,quarantine,config:{trustedSenders:['pauta@fonte-confiavel.example']},mode:'apply',now:fixedNow});
...
test('untrusted Gmail sender is rejected', async () => {
  ...
  const report=await ingestEditorialEntries({source:'gmail',entries:input.messages,repository:memoryRepository(),quarantine:memoryQuarantine(),config:{trustedSenders:['pauta@fonte-confiavel.example']},mode:'dry-run',now:fixedNow});
...
test('SearXNG snippet is explicitly unverified and partial search is warned', async () => {
  ...
  const report=await ingestEditorialEntries({source:'searxng',entries:input.results,repository:memoryRepository(),quarantine:memoryQuarantine(),config:{partial:true,failedEngines:['engine-x']},mode:'dry-run',now:fixedNow});
...
test('dry-run never writes repository or quarantine', async () => {
  const input=await fixture('gmail-trusted.json');
  const repository=memoryRepository(); const quarantine=memoryQuarantine();
  const report=await ingestEditorialEntries({source:'gmail',entries:input.messages,repository,quarantine,config:{trustedSenders:['pauta@fonte-confiavel.example']},mode:'dry-run',now:fixedNow});
...
test('all generated pitches remain preliminary and never approved', async () => {
  const input=await fixture('freshrss.json'); const repository=memoryRepository();
  await ingestEditorialEntries({source:'freshrss',entries:input.items,repository,quarantine:memoryQuarantine(),mode:'apply',now:fixedNow});
  assert.equal(repository.rows.length,2);
  assert.ok(repository.rows.every(p=>p.status==='TRIAGEM'&&p.classificacao_preliminar==='PRELIMINAR_NAO_EDITORIAL'&&p.status!=='APROVADA'));
});

test('source fingerprint includes canonical URL, title and origin', () => {
  const a=createSourceFingerprint({url:'https://example.com/x?utm_source=a',title:'Título',source_origin:'Origem'});
  const b=createSourceFingerprint({url:'https://example.com/x',title:'Título',source_origin:'Origem'});
  const c=createSourceFingerprint({url:'https://example.com/x',title:'Outro',source_origin:'Origem'});
