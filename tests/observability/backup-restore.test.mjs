...
const run = (args, env) => spawnSync('sh', args, { cwd: project, encoding: 'utf8', env: { ...process.env, ...env } });
...
test('backup apply produces only encrypted portable artifacts and restore test succeeds', async () => {
  ...
  try {
    const bin = join(temp, 'bin'); const output = join(temp, 'backups');
    await mkdir(bin); await mkdir(output);
...
`);
    ...
    await writeFile(envFile, `COMPOSE_PROJECT_NAME=subsolo_test
...
`);
    const passphrase = join(temp, 'passphrase'); await writeFile(passphrase, 'fixture-passphrase-very-long\n'); await chmod(passphrase, 0o600);
    const env = { PATH: `${bin}:${process.env.PATH}`, SUBSOLO_DOCKER_BIN: docker, SUBSOLO_ENV_FILE: envFile, SUBSOLO_BACKUP_TIMESTAMP: '20260720T230000Z' };
    const backup = run(['infra/scripts/backup.sh', '--apply', '--output', output, '--passphrase-file', passphrase], env);
    ...
    const restore = run(['infra/scripts/restore.sh', '--test', '--backup-file', encrypted, '--passphrase-file', passphrase, '--report', report], env);
...
test('restore rejects corrupted encrypted artifact', async () => {
  ...
  try {
    const backup = join(temp, 'broken.enc'); const passphrase = join(temp, 'passphrase');
    await writeFile(backup, 'corrupt'); await writeFile(passphrase, 'secret\n');
    const checksum = spawnSync('sha256sum', [backup], { encoding: 'utf8' }).stdout.replace(backup, 'broken.enc');
    await writeFile(`${backup}.sha256`, checksum);
    const result = run(['infra/scripts/restore.sh', '--test', '--backup-file', backup, '--passphrase-file', passphrase], {});
