...
export const walkFiles = async (root, options = {}) => {
  const excluded = new Set(options.excludeDirectories ?? ['.git', 'node_modules', 'dist', '.astro', '.cache', '.tmp']);
  const files = [];
...
export const textExtensions = new Set(['.astro','.css','.csv','.env','.html','.js','.json','.jsx','.md','.mjs','.mts','.py','.sh','.sql','.svg','.toml','.ts','.tsx','.txt','.xml','.yaml','.yml']);
export const isTextCandidate = (path) => textExtensions.has(extname(path).toLowerCase()) || ['Dockerfile','AGENTS.md','README.md','SECURITY.md','CHANGELOG.md'].includes(path.split('/').at(-1));
