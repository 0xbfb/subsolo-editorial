...
const fail = (code, message) => { console.error(`${code}: ${message}`); process.exitCode = 1; };
const json = async (path) => JSON.parse(await readFile(path, 'utf8'));

const typeMatches = (value, type) => type === 'null' ? value === null : type === 'array' ? Array.isArray(value) : type === 'object' ? typeof value === 'object' && value !== null && !Array.isArray(value) : type === 'integer' ? Number.isInteger(value) : typeof value === type;
const validate = (value, schema, path = '$') => {
  const errors = [];
  if (schema.anyOf) {
    if (!schema.anyOf.some((candidate) => validate(value, candidate, path).length === 0)) errors.push(`${path}: nenhum anyOf aceito`);
    return errors;
  }
  ...
  if (schema.type && !typeMatches(value, schema.type)) return [...errors, `${path}: type ${schema.type}`];
  if (typeof value === 'string') {
    ...
    if (schema.format === 'date' && !/^\d{4}-\d{2}-\d{2}$/.test(value)) errors.push(`${path}: date`);
    if (schema.format === 'date-time' && Number.isNaN(Date.parse(value))) errors.push(`${path}: date-time`);
    if (schema.format === 'uri') { try { new URL(value); } catch { errors.push(`${path}: uri`); } }
  }
  ...
  if (schema.type === 'object' && value && typeof value === 'object' && !Array.isArray(value)) {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (schema.properties?.[key]) errors.push(...validate(item, schema.properties[key], `${path}.${key}`));
...
const schemaFiles = (await readdir(schemaRoot)).filter((name) => name.endsWith('.schema.json')).sort();
if (schemaFiles.length !== 13) fail('SUBSOLO_SCHEMA_COUNT_INVALID', `Esperado 13; recebido ${schemaFiles.length}.`);
for (const name of schemaFiles) {
  const schema = await json(`${schemaRoot}/${name}`);
  if (schema.$schema !== 'https://json-schema.org/draft/2020-12/schema') fail('SUBSOLO_SCHEMA_DRAFT_INVALID', name);
  ...
  for (const fixture of mappings[name] ?? []) {
    ...
    values.forEach((value, index) => {
      ...
      if (errors.length) fail('SUBSOLO_SCHEMA_FIXTURE_INVALID', `${name}/${fixture}[${index}]: ${errors.join(', ')}`);
    });
  }
}
if (!process.exitCode) console.log(JSON.stringify({ status: 'passed', schemas: schemaFiles.length, draft: '2020-12' }, null, 2));
