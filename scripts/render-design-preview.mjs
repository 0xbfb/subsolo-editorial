...
const fixture = JSON.parse(
  await readFile(resolve(root, 'src/data/fixtures/jornal-concreto-home.json'), 'utf8'),
);
const channels = JSON.parse(
  await readFile(resolve(root, 'src/data/editorial/catalogs/channels.json'), 'utf8'),
);
...
const stories = (items, compact = false) =>
  items
    .map(
      (item) => `
...
const nav = fixture.navigation
  .map((item) =>
    item.available
      ...
      : `<span aria-disabled="true">${escape(item.label)}</span>`,
  )
  .join('');
...
await writeFile(
  resolve(output, 'assets/theme.js'),
