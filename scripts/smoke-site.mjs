...
if (!input && !fixtureDir) {
  console.error('SUBSOLO_SMOKE_URL_REQUIRED: informe a URL publicada ou --fixture-dir.');
  process.exit(9);
}
const root = input
  ? input.endsWith('/')
    ? input
    : `${input}/`
  : new URL(`file://${resolve(fixtureDir)}/`).href;
...
for (const check of checks) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
        ...
        const relative =
          check.path === ''
            ...
            : check.path.endsWith('/')
              ...
              : check.path;
        body = await readFile(resolve(fixtureDir, relative), 'utf8');
        response = { ok: true, status: 200 };
      } else {
        response = await fetch(url, {
          headers: { 'user-agent': 'subsolo-external-smoke/0.9.0' },
          redirect: 'follow',
      ...
      if (valid && check.kind === 'json') {
        try {
          const parsed = JSON.parse(body);
          valid = Array.isArray(parsed) || typeof parsed === 'object';
        } catch {
          valid = false;
        }
      }
...
const report = {
  ...
  metrics: {
    ...
    total_bytes: results.reduce((sum, item) => sum + item.bytes, 0),
    max_duration_ms: Math.max(...results.map((item) => item.duration_ms)),
  },
};
if (reportPath) {
...
if (!report.ok) {
  console.error('SUBSOLO_SMOKE_FAILED');
