...
const localTarget = (source, href) => {
  ...
  const resolved = pathname.startsWith('/') ? join(root, pathname) : resolve(dirname(source), pathname);
...
for (const { path, stats } of files) {
  ...
  if (/\.map$/i.test(name)) violations.push(`${name}: source map não permitido no artefato inicial`);
  ...
  if (!/<html[^>]+lang=["']pt-BR["']/i.test(html)) violations.push(`${name}: lang pt-BR ausente`);
  if (!/<meta[^>]+name=["']viewport["']/i.test(html)) violations.push(`${name}: viewport ausente`);
  ...
  const cspTag = html.match(/<meta\b[^>]*http-equiv=["']Content-Security-Policy["'][^>]*>/i)?.[0]
    ?? html.match(/<meta\b[^>]*content=(?:"[^"]*"|'[^']*')[^>]*http-equiv=["']Content-Security-Policy["'][^>]*>/i)?.[0];
  const csp = cspTag?.match(/\bcontent=("([^"]*)"|'([^']*)')/i)?.slice(2).find((value) => value !== undefined);
  if (!csp) violations.push(`${name}: CSP meta ausente`);
  else {
    if (/unsafe-inline|(?:^|\s)'unsafe-eval'(?:\s|$)/i.test(csp)) violations.push(`${name}: CSP contém relaxamento proibido`);
  ...
  if (!/<meta[^>]+name=["']referrer["'][^>]+content=["']strict-origin-when-cross-origin["']/i.test(html)
      && !/<meta[^>]+content=["']strict-origin-when-cross-origin["'][^>]+name=["']referrer["']/i.test(html)) violations.push(`${name}: referrer policy ausente`);
  if (!redirect) {
    if (!/<main[^>]+id=["']conteudo["']/i.test(html)) violations.push(`${name}: main#conteudo ausente`);
    if (!/<a[^>]+class=["'][^"']*skip-link[^"']*["'][^>]+href=["']#conteudo["']/i.test(html)) violations.push(`${name}: skip link ausente`);
  }
  const executableInline = [...html.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script>/gi)]
  ...
  if (/<script\b[^>]+type=["']application\/ld\+json["']/i.test(html) && csp && !/sha256-[A-Za-z0-9+/=]+/.test(csp)) violations.push(`${name}: JSON-LD sem hash CSP`);
  for (const image of html.matchAll(/<img\b([^>]*)>/gi)) {
    if (!/\balt=/.test(image[1])) violations.push(`${name}: img sem alt`);
    if (!/\bwidth=/.test(image[1]) || !/\bheight=/.test(image[1])) violations.push(`${name}: img sem dimensões`);
  }
  ...
  for (const href of attributes) {
    ...
    if (!target) continue;
    if (!fileSet.has(resolve(target))) violations.push(`${name}: referência interna ausente ${href}`);
  }
}
...
console.log(`Artefato estático válido: ${htmlCount} páginas, ${files.length} arquivos, ${totalSize} bytes.`);
