...
const forbiddenContent = [
  ...
  [
    'private-editorial-marker',
    /\b(?:SUBSOLO_PRIVATE|NOTA_INTERNA|FONTE_CONFIDENCIAL|NAO_PUBLICAR|NÃO_PUBLICAR)\b/i,
  ],
  [
    'private-google-link',
    /https:\/\/(?:docs|drive)\.google\.com\/(?:document|spreadsheets|file)\/d\//i,
  ],
];
...
const walk = async (directory) => {
  ...
  for (const entry of entries) {
    ...
    if (stats.isSymbolicLink())
      throw new Error(`SUBSOLO_ARTIFACT_SYMLINK: ${relative(root, path)}`);
...
for (const { path, stats } of files) {
  ...
  for (const pattern of forbiddenFiles)
    if (pattern.test(name)) violations.push(`${name}: arquivo proibido`);
...
console.log(
  `Artefato público aprovado: ${files.length} arquivos sem segredos ou marcadores privados detectados.`,
);
