...
for (const monitor of all) {
  if (ids.has(monitor.id)) errors.push(`monitor duplicado: ${monitor.id}`);
  ids.add(monitor.id);
  if (!['http', 'tcp'].includes(monitor.type)) errors.push(`tipo inválido: ${monitor.id}`);
  if (!(monitor.interval_seconds >= 60)) errors.push(`intervalo agressivo: ${monitor.id}`);
  if (monitor.type === 'http' && !monitor.url) errors.push(`URL ausente: ${monitor.id}`);
}
...
if (!/schedule:/.test(scheduled) || !/scripts\/smoke-site\.mjs/.test(scheduled))
  errors.push('smoke externo agendado ausente');
if (errors.length) {
  console.error(errors.join('\n'));
  process.exit(1);
}
console.log(
  `Observabilidade válida: ${all.length} monitores, ${Object.keys(topics.topics).length} tópicos e smoke externo agendado.`,
);
