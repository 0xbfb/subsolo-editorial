...
const parser = await import(pathToFileURL(`${compiledRoot}/application/parse-publication-document.js`));
...
const schemas = Object.fromEntries(await Promise.all(
  ['publication', 'edition', 'source', 'correction', 'redirect', 'tombstone', 'media', 'story', 'author', 'channel', 'topic'].map(async (name) => [name, JSON.parse(await readFile(`schemas/1.0.0/${name}.schema.json`, 'utf8'))]),
));
const enumChecks = [
...
for (const [jsonEnum, domainEnum, field] of enumChecks) {
  if (JSON.stringify(jsonEnum) !== JSON.stringify(domainEnum)) throw new Error(`SUBSOLO_ZOD_JSON_ENUM_DRIFT: ${field}`);
}
const checks = [
...
if (checks.some((check) => !check.ok)) throw new Error(JSON.stringify(checks.filter((check) => !check.ok)));
const referenceIssues = refs.validatePublicReferences({ authors, channels: [channel], topics, stories: [story], publications: [parsed.value.publication], editions: [edition] });
if (referenceIssues.length) throw new Error(JSON.stringify(referenceIssues));
const brokenReferenceIssues = refs.validatePublicReferences({ authors: [], channels: [channel], topics, stories: [story], publications: [parsed.value.publication], editions: [edition] });
if (!brokenReferenceIssues.some((issue) => issue.code === 'SUBSOLO_PUBLICATION_AUTHOR_UNKNOWN')) throw new Error('SUBSOLO_BROKEN_REFERENCE_NOT_DETECTED');
for (let index = 0; index < 128; index += 1) {
  const generated = ids.createEntityId('pub', 1_721_293_200_000 + index, new Uint8Array([0,1,2,3,4,5,6,7,8,index % 256]));
  if (!ids.isEntityId(generated, 'pub')) throw new Error('SUBSOLO_GENERATED_ID_INVALID');
}
const privateField = parser.parsePublicationDocument(await readFile('fixtures/public/invalid/publication-private-field.md', 'utf8'));
if (privateField.ok) throw new Error('SUBSOLO_PRIVATE_FIELD_ACCEPTED');
const dangerous = parser.parsePublicationDocument(await readFile('fixtures/public/invalid/publication-dangerous-url.md', 'utf8'));
if (dangerous.ok) throw new Error('SUBSOLO_UNSAFE_URL_ACCEPTED');
const unsafe = parser.parsePublicationDocument(await readFile('fixtures/public/invalid/publication-unsafe-html.md', 'utf8'));
...
if (slug !== 'aceite-a-nova-regra-ou-desapareca-da-fila') throw new Error('SUBSOLO_SLUG_UNEXPECTED');
console.log(JSON.stringify({ status: 'passed', entities: checks.length + 1, references: 'passed', unsafe_html: 'rejected', unsafe_url: 'rejected', private_field: 'rejected', schema_major: 'rejected', broken_reference: 'rejected', generated_ids: 128, zod_json_enums: enumChecks.length, slug }, null, 2));
