...
async function compareDirectories(left, right) {
  ...
  if (JSON.stringify(a) !== JSON.stringify(b)) throw new Error('Diretórios divergentes no fluxo fixture.');
  return a.length;
}
...
try {
  ...
  run('export-dry-run', process.execPath, ['cli/subsolo.mjs', 'export-doc', '--provider', 'fixture', '--sheet', 'fixtures/exporter/valid/sheet-row.json', '--document', 'fixtures/exporter/valid/document.json', '--destination', exportA, '--dry-run']);
  run('export-apply-a', process.execPath, ['cli/subsolo.mjs', 'export-doc', '--provider', 'fixture', '--sheet', 'fixtures/exporter/valid/sheet-row.json', '--document', 'fixtures/exporter/valid/document.json', '--destination', exportA, '--apply']);
  run('export-apply-b', process.execPath, ['cli/subsolo.mjs', 'export-doc', '--provider', 'fixture', '--sheet', 'fixtures/exporter/valid/sheet-row.json', '--document', 'fixtures/exporter/valid/document.json', '--destination', exportB, '--apply']);
  ...
  run('package-dry-run', process.execPath, ['cli/subsolo.mjs', 'package', '--workspace', 'fixtures/packager/edition-r1', '--destination', packageA, '--dry-run']);
  run('package-apply-a', process.execPath, ['cli/subsolo.mjs', 'package', '--workspace', 'fixtures/packager/edition-r1', '--destination', packageA, '--apply']);
  run('package-apply-b', process.execPath, ['cli/subsolo.mjs', 'package', '--workspace', 'fixtures/packager/edition-r1', '--destination', packageB, '--apply']);
  ...
  run('validate-package', process.execPath, ['cli/subsolo.mjs', 'validate-package', '--package', zipA]);
  const restored = join(work, 'restored');
  run('restore-dry-run', process.execPath, ['cli/subsolo.mjs', 'restore', '--package', zipA, '--destination', restored, '--dry-run']);
  run('restore-apply', process.execPath, ['cli/subsolo.mjs', 'restore', '--package', zipA, '--destination', restored, '--apply']);
  ...
  ]) run(`validate-${basename(editionPackage)}`, process.execPath, ['cli/subsolo.mjs', 'validate-package', '--package', editionPackage]);

  run('orchestrate-dry-run', process.execPath, ['cli/subsolo.mjs', 'orchestrate', '--input', 'fixtures/n8n/publication-candidate.json', '--dry-run']);
  run('reconcile-consistent', process.execPath, ['cli/subsolo.mjs', 'reconcile', '--input', 'fixtures/observability/reconciliation-consistent.json', '--dry-run']);

  const mediaOutput = join(work, 'media');
  run('media-dry-run', process.execPath, ['cli/subsolo.mjs', 'media', '--manifest', 'fixtures/media/portrait/manifest.json', '--destination', mediaOutput, '--dry-run']);
  run('media-apply', process.execPath, ['cli/subsolo.mjs', 'media', '--manifest', 'fixtures/media/portrait/manifest.json', '--destination', mediaOutput, '--apply']);
  ...
  for (const name of ['n8n_data', 'freshrss_data', 'freshrss_extensions', 'uptime_kuma_data']) {
    ...
    run(`backup-volume-${name}`, 'tar', ['-czf', join(backupStaging, `${name}.tar.gz`), '-C', volumeDir, '.']);
  }
  await writeFile(join(backupStaging, 'runtime.json'), `${JSON.stringify({ schema_version: 1, product_version: '1.0.0-pre', secrets_included: false }, null, 2)}\n`);
  run('backup-manifest-create', process.execPath, ['scripts/backup-manifest.mjs', 'create', '--directory', backupStaging, '--created-at', '2026-07-21T12:00:00Z', '--product-version', '1.0.0-pre']);
  run('backup-manifest-validate', process.execPath, ['scripts/backup-manifest.mjs', 'validate', '--directory', backupStaging]);
  ...
  run('backup-encrypt', 'openssl', ['enc', '-aes-256-cbc', '-pbkdf2', '-salt', '-in', plainBackup, '-out', encryptedBackup, '-pass', `file:${passphrase}`]);
  ...
  run('backup-restore-test', 'sh', ['infra/scripts/restore.sh', '--test', '--backup-file', encryptedBackup, '--passphrase-file', passphrase, '--report', restoreReport]);
  ...
  console.log(`Fluxo fixture aprovado: ${steps.length} etapas, pacote ${packageName}, restore ${restore.status}.`);
} finally {
