...
const loadLocal = async (packagePath) => {
  ...
  try {
    ...
    fail(
      'SUBSOLO_ARCHIVE_PACKAGE_NOT_FOUND',
      `Pacote não encontrado: ${absolute}.`,
      'Gere ou informe um ZIP válido.',
    );
  }
  let inspected;
  try {
    inspected = inspectPackage(bytes);
  } catch (error) {
    fail(
      'SUBSOLO_ARCHIVE_PACKAGE_INVALID',
      'O arquivo local não é um pacote Subsolo íntegro.',
      'Execute validate-package antes do upload.',
  ...
  if (!date)
    fail(
      'SUBSOLO_ARCHIVE_EDITION_DATE_INVALID',
      'A edição não possui data válida.',
      'Corrija edition_date antes de empacotar.',
    );
  return {
    absolute,
    bytes,
...
const receiptDestination = (packagePath, configured) =>
  path.resolve(configured ?? `${packagePath}.drive-receipt.json`);
const drivePath = (year, month, fileName) =>
  `SUBSOLO/90_ARQUIVO_TECNICO/edicoes/${year}/${month}/${fileName}`;
...
const validateRemoteMetadata = ({ remote, local, folderId }) => {
  ...
  if (metadataField(remote, 'subsolo_package_sha256') !== local.packageSha256)
    errors.push('package_sha256');
  if (metadataField(remote, 'subsolo_edition_id') !== String(local.manifest.edition_id))
    errors.push('edition_id');
  if (metadataField(remote, 'subsolo_run_id') !== String(local.manifest.run_id))
    errors.push('run_id');
  if (metadataField(remote, 'subsolo_revision') !== String(local.manifest.revision))
    errors.push('revision');
  if (metadataField(remote, 'subsolo_schema_version') !== String(local.manifest.schema_version))
  ...
  if (errors.length) {
    fail(
      'SUBSOLO_ARCHIVE_REMOTE_METADATA_INVALID',
      'O arquivo remoto não corresponde ao pacote local.',
      'Não registre o file ID; revise o upload e tente novamente.',
      { fields: errors },
    );
  }
};
...
const verifyDownloadedPackage = ({ downloaded, local }) => {
  ...
  if (sha256(buffer) !== local.packageSha256) {
    fail(
      'SUBSOLO_ARCHIVE_DOWNLOAD_CHECKSUM_INVALID',
      'O download de verificação diverge do pacote local.',
      'Mantenha o pacote local e repita a preservação.',
      {},
      true,
    );
  }
  let inspected;
  try {
    inspected = inspectPackage(buffer);
  } catch (error) {
    fail(
      'SUBSOLO_ARCHIVE_DOWNLOAD_PACKAGE_INVALID',
      'O arquivo baixado não restaura como pacote Subsolo.',
      'Repita o upload e a verificação.',
      { cause: error?.code ?? 'unknown' },
      true,
    );
  }
  if (
    inspected.manifest.edition_id !== local.manifest.edition_id ||
    inspected.manifest.run_id !== local.manifest.run_id
  ) {
    fail(
      'SUBSOLO_ARCHIVE_DOWNLOAD_MANIFEST_INVALID',
      'O manifesto baixado pertence a outra execução.',
      'Não registre o file ID e investigue a divergência.',
    );
  }
};
...
export const planPackageArchive = async ({
  packagePath,
  rootFolderId,
  receiptPath = null,
  adapter,
  ...
  const duplicate = target.folderId
    ? await adapter.findByPackageSha256(target.folderId, local.packageSha256)
    : null;
  return {
    mode: 'dry-run',
...
export const applyPackageArchive = async ({
  packagePath,
  rootFolderId,
  receiptPath = null,
  adapter,
