const SECRET_KEY =
  /(?:authorization|access[_-]?token|refresh[_-]?token|password|secret|private[_-]?key|cookie|session|credential|encryption[_-]?key)/i;
const BODY_KEY =
  /^(?:body|content|article_body|document_text|editorial_text|raw_document|markdown)$/i;
const TOKEN_VALUE =
  /(?:Bearer\s+[A-Za-z0-9._~+\/-]+=*|ya29\.[A-Za-z0-9._-]+|gh[pousr]_[A-Za-z0-9_]+|-----BEGIN [A-Z ]*PRIVATE KEY-----)/gi;
...
export const classifyRetryable = ({ code = '', status = null, cause = '' } = {}) => {
  ...
  return /(?:TIMEOUT|NETWORK|ECONNRESET|EAI_AGAIN|RATE_LIMIT|QUOTA|TEMPORARY|UNAVAILABLE|LOCKED)/i.test(
    `${code} ${cause}`,
  );
};

export const createOperationalLogger = ({
  service,
  sink = (entry) => process.stderr.write(`${JSON.stringify(entry)}\n`),
  clock = () => new Date(),
  runId = null,
} = {}) => {
...
export const createExecutionMetrics = ({
  startedAt,
  finishedAt,
  status,
  artifacts = [],
  ...
  if (Number.isNaN(started.valueOf()) || Number.isNaN(finished.valueOf()) || finished < started)
    throw new Error('SUBSOLO_METRICS_TIME_INVALID');
