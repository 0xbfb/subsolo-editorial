...
export const archivePage = (page: number) =>
  paginateEntries(discoveryEntries, page, archivePageSize);
...
export const archiveEntriesForChannel = (slug: string) =>
  discoveryEntries.filter((entry) => entry.channel === slug);
export const archiveEntriesForTopic = (slug: string) =>
  discoveryEntries.filter((entry) => entry.topics.includes(slug));
export const archiveEntriesForAuthor = (slug: string) =>
  discoveryEntries.filter((entry) => entry.authors.includes(slug) || entry.editor === slug);
export const archiveEntriesForStory = (slug: string) =>
  discoveryEntries.filter((entry) => entry.story === slug);
export const archiveRecordBySlug = Object.freeze(
  Object.fromEntries(archiveRecords.map((entry) => [entry.slug, entry])),
);
