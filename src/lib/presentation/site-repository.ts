...
export const authors = Object.freeze(authorCatalog.map((author) => ({ ...author, portrait: portraitForAuthor(portraitRegistry, author.slug) })));
...
export const authorBySlug = Object.freeze(Object.fromEntries(authors.map((author) => [author.slug, author])));
export const channelBySlug = Object.freeze(Object.fromEntries(channels.map((channel) => [channel.slug, channel])));
export const topicBySlug = Object.freeze(Object.fromEntries(topics.map((topic) => [topic.slug, topic])));

export const channelNames = Object.freeze(Object.fromEntries(channels.map((channel) => [channel.slug, channel.name])));
export const authorNames = Object.freeze(Object.fromEntries(authors.map((author) => [author.slug, author.nickname])));
...
export const publicationsByAuthor = (slug: string) =>
  editorialSite.publications.filter((publication) => publication.authors.includes(slug) || publication.editor === slug);
