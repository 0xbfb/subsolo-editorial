...
export const publicPublicationPathSchema = z
  .string()
  .regex(/^\/\d{4}\/\d{2}\/\d{2}\/[a-z0-9]+(?:-[a-z0-9]+)*\/$/);

export const publicRedirectSchema = z
  .object({
    schema_version: schemaVersion,
  ...
  .refine((value) => value.from_path !== value.to_path, {
    message: 'Redirect não pode apontar para si próprio.',
  });
      ...
      .array(
        z
          .object({
            ...
            mime_type: z
              .string()
              .regex(/^image\/(?:avif|webp|jpeg|png)$/)
              .optional(),
