...
export const migrateSchemaDocument = (
  ...
  if (typeof input !== 'object' || input === null || Array.isArray(input)) {
    return {
      ...
      issues: [
        issue(
          'SUBSOLO_SCHEMA_DOCUMENT_INVALID',
          'Documento versionado precisa ser objeto.',
          'Forneça um objeto JSON com schema_version.',
        ),
      ],
    };
  }
  ...
  if (typeof document.schema_version !== 'string') {
    return {
      ...
      issues: [
        issue(
          'SUBSOLO_SCHEMA_VERSION_MISSING',
  ...
  if (!current || !target) {
    return {
      ...
      issues: [
        issue(
          'SUBSOLO_SCHEMA_VERSION_INVALID',
          'Versão de schema inválida.',
          'Use versionamento semântico X.Y.Z.',
        ),
      ],
    };
  }
  if (current.major > target.major) {
    return {
      ...
      issues: [
        issue(
          'SUBSOLO_SCHEMA_MAJOR_UNSUPPORTED',
          `Schema ${document.schema_version} é mais novo que ${targetVersion}.`,
          'Atualize o importador antes de publicar.',
        ),
      ],
    };
  }
  ...
  while (migrated.schema_version !== targetVersion) {
    if (visited.has(migrated.schema_version)) {
      return {
        ...
        issues: [
          issue(
            'SUBSOLO_SCHEMA_MIGRATION_CYCLE',
            'Ciclo detectado no migrador.',
            'Revise o registro de migrações.',
          ),
        ],
      };
    }
    ...
    if (!migration) {
      return {
        ...
        issues: [
          issue(
            'SUBSOLO_SCHEMA_MIGRATION_MISSING',
            `Não existe migração de ${migrated.schema_version} para ${targetVersion}.`,
            'Adicione uma migração explícita; não altere o documento silenciosamente.',
          ),
        ],
      };
    }
