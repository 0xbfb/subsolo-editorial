...
const FILE_FIELDS =
  'id,name,mimeType,parents,appProperties,size,md5Checksum,createdTime,modifiedTime,trashed,driveId';
...
const fail = (code, message, action, details = {}, retryable = false) => {
  throw new GoogleDriveArchiveFailure(code, message, action, details, retryable);
};
...
const failureFromResponse = async (response, context = {}) => {
  ...
  if (response.status === 401)
    return new GoogleDriveArchiveFailure(
      'SUBSOLO_DRIVE_AUTH_INVALID',
      'A credencial do Drive foi recusada.',
      'Renove a credencial e confirme o scope do Drive.',
      { ...context, status: response.status, reason },
    );
  if (response.status === 403 && ['rateLimitExceeded', 'userRateLimitExceeded'].includes(reason))
    return new GoogleDriveArchiveFailure(
      'SUBSOLO_DRIVE_QUOTA_EXCEEDED',
      'O Drive limitou temporariamente as requisições.',
      'Aguarde o retry automático ou reduza a frequência.',
      { ...context, status: response.status, reason },
      true,
    );
  if (response.status === 403)
    return new GoogleDriveArchiveFailure(
      'SUBSOLO_DRIVE_ACCESS_DENIED',
      'A pasta ou arquivo não está autorizado para esta credencial.',
      'Compartilhe a raiz técnica ou use uma credencial com acesso explícito.',
      { ...context, status: response.status, reason },
    );
  if (response.status === 404)
    return new GoogleDriveArchiveFailure(
      'SUBSOLO_DRIVE_RESOURCE_NOT_FOUND',
      'O recurso do Drive não foi encontrado.',
      'Confirme o ID, compartilhamento e se a sessão resumível ainda é válida.',
      { ...context, status: response.status, reason },
    );
  ...
  if (response.status >= 500)
    return new GoogleDriveArchiveFailure(
      'SUBSOLO_DRIVE_UPSTREAM_UNAVAILABLE',
      'O Google Drive está temporariamente indisponível.',
      'Tente novamente sem apagar o pacote local.',
      { ...context, status: response.status, reason },
      true,
    );
  return new GoogleDriveArchiveFailure(
    'SUBSOLO_DRIVE_HTTP_ERROR',
    `O Drive respondeu com HTTP ${response.status}.`,
    'Revise a requisição e a configuração.',
    { ...context, status: response.status, reason },
  );
};
    ...
    try {
      ...
      if (!accepted.has(response.status)) {
        ...
        if (failure.retryable && attempt < maxAttempts) {
          last = failure;
          logger.warn('google.drive.retry', {
            attempt,
      ...
      const failure = new GoogleDriveArchiveFailure(
        timeout ? 'SUBSOLO_DRIVE_TIMEOUT' : 'SUBSOLO_DRIVE_NETWORK_ERROR',
        timeout
          ? 'A chamada ao Drive excedeu o tempo limite.'
          : 'Falha de rede ao acessar o Drive.',
        timeout
          ? 'Aumente o timeout somente se necessário.'
...
export const createGoogleDriveTokenProvider = ({
  ...
  if (env.GOOGLE_DRIVE_ACCESS_TOKEN?.trim())
    return createEnvironmentAccessTokenProvider({
      env: { GOOGLE_WORKSPACE_ACCESS_TOKEN: env.GOOGLE_DRIVE_ACCESS_TOKEN },
    });
  ...
  const scope =
    configured === GOOGLE_DRIVE_FULL_SCOPE ? GOOGLE_DRIVE_FULL_SCOPE : GOOGLE_DRIVE_FILE_SCOPE;
...
const parseRangeNextOffset = (header, total) => {
  ...
  if (!match)
    fail(
      'SUBSOLO_DRIVE_UPLOAD_RANGE_INVALID',
      'A sessão resumível retornou um Range inválido.',
      'Reinicie o upload mantendo o pacote local.',
      { header: sha256(header).slice(0, 12) },
      true,
    );
...
export const createGoogleDriveArchiveAdapter = ({
  tokenProvider = createGoogleDriveTokenProvider(),
  fetchImpl = globalThis.fetch,
  timeoutMs = 20_000,
  maxAttempts = 4,
  chunkSize = 8 * 256 * 1024,
  logger = noOpLogger,
  sleep = sleepDefault,
  ...
  if (!Number.isInteger(chunkSize) || chunkSize < 256 * 1024 || chunkSize % (256 * 1024) !== 0) {
    fail(
      'SUBSOLO_DRIVE_CHUNK_SIZE_INVALID',
      'O chunk do upload deve ser múltiplo de 256 KiB.',
      'Use 262144 bytes ou um múltiplo inteiro.',
    );
  }
  const request = (input) =>
    authorizedFetch({ tokenProvider, fetchImpl, timeoutMs, maxAttempts, logger, sleep, ...input });
  ...
  const validateRoot = async (rootFolderId) => {
    ...
    if (root.mimeType !== FOLDER_MIME || root.trashed === true)
      fail(
        'SUBSOLO_DRIVE_ROOT_INVALID',
        'A raiz configurada não é uma pasta ativa.',
        'Informe o ID da pasta SUBSOLO autorizada.',
      );
  ...
  const listFiles = async (query) => {
    ...
    do {
      ...
      if (rootDriveId) {
        ...
        url.searchParams.set('driveId', rootDriveId);
      }
      if (pageToken) url.searchParams.set('pageToken', pageToken);
      const page = (await request({ url: url.toString(), context: { operation: 'files.list' } }))
        .data;
  ...
  const findChildFolder = async (parentId, name) => {
    ...
    if (files.length > 1)
      fail(
        'SUBSOLO_DRIVE_FOLDER_AMBIGUOUS',
        `Há mais de uma pasta chamada ${name} sob o mesmo pai.`,
        'Remova a ambiguidade antes da publicação.',
        { count: files.length },
      );
    return files[0] ?? null;
  };
  ...
  const createFolder = async (parentId, name, logicalPath) => {
    ...
    const body = JSON.stringify({
      ...
      parents: [parentId],
      appProperties: {
        subsolo_kind: 'archive-folder',
        subsolo_path_sha256: sha256(logicalPath).slice(0, 40),
      },
    });
    return (
      await request({
        ...
        body,
        accepted: new Set([200, 201]),
        context: { operation: 'folders.create' },
      })
    ).data;
  };
  ...
  const traverse = async (rootFolderId, year, month, apply) => {
    ...
    for (const name of names) {
      if (missing && !apply) {
        segments.push({ name, status: 'create', folderId: null });
        continue;
      }
      ...
      if (!folder) {
        ...
        folder = await createFolder(
          parentId,
          name,
          [...names.slice(0, segments.length + 1)].join('/'),
        );
        status = 'create';
      }
      if (!Array.isArray(folder.parents) || !folder.parents.includes(parentId))
        fail(
          'SUBSOLO_DRIVE_FOLDER_PARENT_INVALID',
          `A pasta ${name} não pertence ao caminho esperado.`,
          'Revise a hierarquia antes do upload.',
        );
      segments.push({ name, status, folderId: folder.id });
      parentId = folder.id;
    }
    return {
      folderId: missing ? null : parentId,
      path: `90_ARQUIVO_TECNICO/edicoes/${year}/${month}`,
      segments,
    };
  };
  ...
  const findByPackageSha256 = async (folderId, packageSha256) => {
    ...
    if (files.length > 1)
      fail(
        'SUBSOLO_DRIVE_DUPLICATE_AMBIGUOUS',
        'Mais de um arquivo remoto possui o mesmo checksum.',
        'Interrompa e audite o arquivo técnico.',
        { count: files.length },
      );
    return files[0] ?? null;
  };
  ...
  const sessionStatus = async (sessionUrl, total) => {
    const response = await request({
      ...
      body: new Uint8Array(),
      accepted: new Set([200, 201, 308]),
      responseType: 'raw',
      context: { operation: 'upload.status' },
    });
    if (response.status === 200 || response.status === 201)
      return { completed: true, metadata: JSON.parse(await response.text()) };
    return {
      completed: false,
      nextOffset: parseRangeNextOffset(response.headers.get('range'), total),
    };
  };
  ...
  const uploadResumable = async ({ folderId, name, bytes, metadata }) => {
    ...
    const initUrl = new URL(`${uploadBaseUrl}/files`);
    ...
    const init = await request({
      ...
      accepted: new Set([200, 201]),
      responseType: 'raw',
      context: { operation: 'upload.initiate' },
    });
    const sessionUrl = init.headers.get('location');
    if (!sessionUrl)
      fail(
        'SUBSOLO_DRIVE_UPLOAD_SESSION_MISSING',
        'O Drive não retornou a URI da sessão resumível.',
        'Repita o upload sem remover o pacote local.',
        {},
        true,
      );
    let offset = 0;
    ...
    while (offset < total) {
      ...
      try {
        const response = await request({
          ...
          responseType: 'raw',
          maxAttempts: 1,
          context: { operation: 'upload.chunk' },
        });
        if (response.status === 200 || response.status === 201)
        ...
        if (next <= offset)
          fail(
            'SUBSOLO_DRIVE_UPLOAD_STALLED',
            'A sessão resumível não avançou.',
            'Reinicie o upload mantendo o pacote local.',
            { offset },
            true,
          );
        ...
        if (
          !(error instanceof GoogleDriveArchiveFailure) ||
          !error.retryable ||
          interruptions >= maxAttempts - 1
        )
          throw error;
        interruptions += 1;
        await sleep(retryDelay(interruptions));
    ...
    if (state.completed) return state.metadata;
    fail(
      'SUBSOLO_DRIVE_UPLOAD_INCOMPLETE',
      'O upload resumível terminou sem confirmação final.',
      'Consulte a sessão e repita a verificação.',
  ...
  const download = async (fileId) => {
    ...
    return (
      await request({
        url: url.toString(),
        responseType: 'buffer',
        context: { operation: 'files.download' },
      })
    ).data;
  };
