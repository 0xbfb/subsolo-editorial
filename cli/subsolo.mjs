...
const requiredValue = (flag, code = 'SUBSOLO_CLI_ARGUMENT_MISSING') => {
  ...
  if (!result)
    throw new ExportFailure(code, `Argumento obrigatório ausente: ${flag}.`, `Informe ${flag}.`);
  return result;
};
    ...
    if (command === 'package') {
      ...
      if (has('--dry-run') === has('--apply')) {
        throw new ExportFailure(
          'SUBSOLO_CLI_MODE_INVALID',
          'Escolha exatamente um modo.',
          'Use --dry-run ou --apply.',
        );
      }
      const options = {
        workspace,
        destination,
        previousPackage: value('--previous-package') ?? null,
      };
      ...
      if (!has('--skip-git')) {
        throw new ExportFailure(
          'SUBSOLO_PUBLISH_GIT_SCOPE_INVALID',
          'O Prompt 11 permite somente preservação no Drive.',
          'Adicione --skip-git; branch e PR pertencem ao Prompt 12.',
        );
      }
      if (has('--dry-run') === has('--apply')) {
        throw new ExportFailure(
      ...
      if (!rootFolderId) {
        throw new ExportFailure(
          'SUBSOLO_ARCHIVE_ROOT_ID_MISSING',
          'A pasta raiz do Drive não foi configurada.',
          'Informe --drive-root-id ou SUBSOLO_DRIVE_ROOT_FOLDER_ID.',
        );
      }
      const { planPackageArchive, applyPackageArchive } =
        await import('../src/lib/application/archive-edition-package.mjs');
      const { createGoogleDriveArchiveAdapter, createGoogleDriveTokenProvider } =
        await import('../src/lib/infrastructure/google/google-drive.mjs');
      const { createStructuredLogger } =
        await import('../src/lib/infrastructure/google/google-workspace.mjs');
      ...
      const adapter = createGoogleDriveArchiveAdapter({
        ...
        logger,
        timeoutMs: Number(
          value('--timeout-ms') ?? process.env.SUBSOLO_GOOGLE_DRIVE_TIMEOUT_MS ?? 20_000,
        ),
        maxAttempts: Number(
          value('--max-attempts') ?? process.env.SUBSOLO_GOOGLE_DRIVE_MAX_ATTEMPTS ?? 4,
        ),
        chunkSize: Number(
          value('--chunk-size') ?? process.env.SUBSOLO_GOOGLE_DRIVE_CHUNK_SIZE ?? 2_097_152,
        ),
      });
      const options = {
        packagePath,
      ...
      const result = has('--dry-run')
        ? await planPackageArchive(options)
        : await applyPackageArchive(options);
      ...
      const payload = value('--payload')
        ? JSON.parse(await readFile(value('--payload'), 'utf8'))
        : {};
      const { executeEditionCommand, toPublicEdition } =
        await import('../src/lib/application/edition-lifecycle.mjs');
      const next = executeEditionCommand({ state, command: editionCommand, payload });
      const result = {
        mode: has('--dry-run') ? 'dry-run' : 'apply',
        command: editionCommand,
        state: next,
        public: next.revision > 0 ? toPublicEdition(next) : null,
      ...
      if (
        ['publish', 'revise', 'seal', 'correct'].includes(editionCommand) &&
        value('--package-workspace') &&
        value('--package-destination')
      ) {
        const { packageLifecycleRevision } =
          await import('../src/lib/application/edition-lifecycle-package.mjs');
        result.package = await packageLifecycleRevision({
          state: next,
          workspaceTemplate: value('--package-workspace'),
          destination: value('--package-destination'),
          previousPackage: value('--previous-package') ?? null,
      ...
      const input = JSON.parse(
        await readFile(requiredValue('--input', 'SUBSOLO_ORCHESTRATION_INPUT_MISSING'), 'utf8'),
      );
      if (has('--dry-run') === has('--apply'))
        throw new ExportFailure(
          'SUBSOLO_CLI_MODE_INVALID',
      ...
      if (command === 'reconcile' && input.kind === 'operational-reconciliation') {
        const { executeOperationalReconciliation } =
          await import('../src/lib/application/operational-reconciliation.mjs');
        result = executeOperationalReconciliation({ snapshot: input, mode });
      } else {
        const { createFixtureOrchestrationPorts } =
          await import('../src/lib/infrastructure/orchestration/fixture-ports.mjs');
        const { executePublicationRun, reconcilePublication } =
          await import('../src/lib/application/publication-orchestrator.mjs');
      ...
      const { planPostPublicationChange, applyPostPublicationChange } =
        await import('../src/lib/domain/post-publication.mjs');
      if (has('--dry-run')) {
      ...
      const statePath =
        value('--state') ??
        process.env.SUBSOLO_INGESTION_STATE ??
        '.runtime/editorial/pitches.json';
      if (has('--dry-run') === has('--apply'))
        throw new ExportFailure(
          'SUBSOLO_CLI_MODE_INVALID',
          'Escolha exatamente um modo.',
      ...
      const readOptionalJson = async (file, fallback = {}) =>
        file ? JSON.parse(await readFile(file, 'utf8')) : fallback;
      const policy = await readOptionalJson(
        value('--policy') ?? process.env.SUBSOLO_INGESTION_POLICY_FILE,
        {},
      );
      const config = {
        trustedSenders:
          policy.trusted_senders ??
          String(process.env.SUBSOLO_GMAIL_TRUSTED_SENDERS ?? '')
            .split(',')
            .map((x) => x.trim())
            .filter(Boolean),
        trustedDomains:
          policy.trusted_domains ??
          String(process.env.SUBSOLO_GMAIL_TRUSTED_DOMAINS ?? '')
            .split(',')
            .map((x) => x.trim())
            .filter(Boolean),
      ...
      if (provider === 'fixture') {
        const input = await readOptionalJson(
          requiredValue('--input', 'SUBSOLO_INGEST_INPUT_MISSING'),
        );
        entries =
          source === 'freshrss'
            ? (input.items ?? [])
        ...
        sourceMeta =
          source === 'searxng'
            ? {
                partial: Boolean(input.unresponsive_engines?.length),
                failedEngines: (input.unresponsive_engines ?? []).map((x) =>
                  Array.isArray(x) ? x[0] : String(x),
                ),
                query: input.query ?? '',
              }
            : {};
      } else if (provider === 'live') {
        const { createFreshRssClient, createGmailIngestionClient, createSearxngClient } =
          await import('../src/lib/infrastructure/ingestion/source-clients.mjs');
        if (source === 'freshrss') {
          const client = createFreshRssClient({
            baseUrl: process.env.SUBSOLO_FRESHRSS_API_URL,
            username: process.env.SUBSOLO_FRESHRSS_API_USER,
            apiPassword: process.env.SUBSOLO_FRESHRSS_API_PASSWORD,
            authToken: process.env.SUBSOLO_FRESHRSS_AUTH_TOKEN,
            timeoutMs: Number(value('--timeout-ms') ?? 10_000),
          });
          const result = await client.listReadingList({ limit: Number(value('--limit') ?? 100) });
          entries = result.entries;
        } else if (source === 'gmail') {
          const client = createGmailIngestionClient({
            accessToken: process.env.SUBSOLO_GMAIL_READONLY_TOKEN,
            timeoutMs: Number(value('--timeout-ms') ?? 10_000),
          });
          const list = await client.listMessages({
            query:
              value('--query') ??
              process.env.SUBSOLO_GMAIL_QUERY ??
              'label:SUBSOLO/Pautas newer_than:2d',
            maxResults: Number(value('--limit') ?? 100),
          });
          entries = await Promise.all(
            (list.messages ?? []).map((item) => client.getMessage(item.id)),
          );
        } else if (source === 'searxng') {
          const client = createSearxngClient({
            baseUrl: process.env.SUBSOLO_SEARXNG_URL ?? 'http://127.0.0.1:8081',
            timeoutMs: Number(value('--timeout-ms') ?? 10_000),
          });
          ...
          if (!query?.trim())
            throw new ExportFailure(
              'SUBSOLO_INGEST_SEARXNG_QUERY_MISSING',
              'Consulta SearXNG ausente.',
              'Informe --query ou um JSON seguro em --query-file.',
            );
          const result = await client.search({
            query,
            categories: value('--categories') ?? queryPayload.categories ?? '',
            language: value('--language') ?? queryPayload.language ?? 'pt-BR',
            timeRange: value('--time-range') ?? queryPayload.time_range ?? null,
          });
          entries = result.entries;
          sourceMeta = result;
        } else
          throw new ExportFailure(
            'SUBSOLO_INGEST_SOURCE_INVALID',
            `Fonte desconhecida: ${source}.`,
            'Use freshrss, gmail ou searxng.',
          );
      } else
        throw new ExportFailure(
          'SUBSOLO_CLI_PROVIDER_INVALID',
          `Provider desconhecido: ${provider}.`,
          'Use fixture ou live.',
        );
      const workbook = JSON.parse(
        await readFile(
          new URL('../templates/google/sheets/workbook.schema.json', import.meta.url),
          'utf8',
        ),
      );
      ...
      if (spreadsheetId && provider === 'live') {
        const { createGoogleSheetsPitchRepository, createIngestionTokenProvider } =
          await import('../src/lib/infrastructure/ingestion/google-sheets-pitch-writer.mjs');
        const { createStructuredLogger } =
          await import('../src/lib/infrastructure/google/google-workspace.mjs');
        const logger = createStructuredLogger({
          sink: structuredStderr,
          verbose: has('--verbose'),
        });
        repository = createGoogleSheetsPitchRepository({
          spreadsheetId,
          range: value('--sheet-range') ?? 'PAUTAS!A:AZ',
          headers,
          tokenProvider: createIngestionTokenProvider({ logger }),
          logger,
        });
      } else {
        const { createFilePitchRepository } =
          await import('../src/lib/infrastructure/ingestion/file-ingestion-store.mjs');
        repository = createFilePitchRepository({ statePath });
      }
      const { createFileQuarantine } =
        await import('../src/lib/infrastructure/ingestion/file-ingestion-store.mjs');
      const quarantine = createFileQuarantine({ directory: path.resolve(output, 'quarantine') });
      const { ingestEditorialEntries, triageReportMarkdown } =
        await import('../src/lib/application/editorial-ingestion.mjs');
      const report = await ingestEditorialEntries({
        ...
        quarantine,
        mode,
        config: { ...config, ...sourceMeta },
      });
      if (mode === 'apply') {
        await mkdir(output, { recursive: true });
        await writeFile(
          path.resolve(output, 'triage-report.json'),
        ...
        JSON.stringify(
          {
            ...
            provider,
            output: path.resolve(output),
            sheets_write: Boolean(spreadsheetId && provider === 'live'),
          },
          null,
          2,
        ),
      ...
      const manifestPath = requiredValue('--manifest', 'SUBSOLO_MEDIA_MANIFEST_MISSING');
      const destination = requiredValue('--destination', 'SUBSOLO_MEDIA_DESTINATION_MISSING');
      const { createPillowMediaProcessor } =
        await import('../src/lib/infrastructure/media/pillow-media-processor.mjs');
      const { planMediaProcessing, applyMediaProcessing } =
        await import('../src/lib/application/process-media.mjs');
      ...
      const result = has('--dry-run')
        ? await planMediaProcessing(options)
        : await applyMediaProcessing(options);
      console.log(JSON.stringify(result, null, 2));
      ...
      if (provider === 'fixture') {
        ...
        const spreadsheetId =
          value('--spreadsheet-id') ?? process.env.SUBSOLO_GOOGLE_SPREADSHEET_ID;
        if (!spreadsheetId) {
          throw new ExportFailure(
        ...
        const {
          createDefaultGoogleTokenProvider,
        ...
        const common = {
          ...
          logger,
          timeoutMs: Number(
            value('--timeout-ms') ?? process.env.SUBSOLO_GOOGLE_TIMEOUT_MS ?? 10_000,
          ),
          maxAttempts: Number(
            value('--max-attempts') ?? process.env.SUBSOLO_GOOGLE_MAX_ATTEMPTS ?? 3,
          ),
          cacheTtlMs: Number(process.env.SUBSOLO_GOOGLE_CACHE_TTL_MS ?? 0),
        };
        const sheetProvider = createGoogleSheetsProvider({
          ...common,
          spreadsheetId,
          range:
            value('--sheet-range') ?? process.env.SUBSOLO_GOOGLE_SHEETS_RANGE ?? 'ARTIGOS!A:AZ',
          pageSize: Number(
            value('--page-size') ?? process.env.SUBSOLO_GOOGLE_SHEETS_PAGE_SIZE ?? 200,
          ),
        });
        const documentProvider = createGoogleDocsProvider({
          ...common,
          tabId: value('--tab-id') ?? process.env.SUBSOLO_GOOGLE_DOCS_TAB_ID ?? null,
        });
        ...
        throw new ExportFailure(
          'SUBSOLO_CLI_PROVIDER_INVALID',
          `Provider desconhecido: ${provider}.`,
          'Use fixture ou google.',
        );
      }
      console.log(JSON.stringify(result, null, 2));
    } else {
      throw new ExportFailure(
        'SUBSOLO_CLI_COMMAND_INVALID',
        'Comando ausente ou inválido.',
        'Use export-doc, media, package, restore, validate-package, publish, orchestrate, reconcile, edition, ingest ou post-publication.',
      );
    }
  }
} catch (error) {
  const payload =
    typeof error?.toJSON === 'function'
      ...
      : error instanceof ExportFailure
        ? error.toJSON()
        : {
            code: 'SUBSOLO_UNEXPECTED',
            message: error instanceof Error ? error.message : String(error),
            action: 'Consulte o log e corrija a entrada.',
          };
